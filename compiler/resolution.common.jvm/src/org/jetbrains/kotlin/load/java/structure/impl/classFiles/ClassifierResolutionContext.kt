...
class ClassifierResolutionContext private constructor(
    ...
    private var typeParameters: SmartFMap<String, JavaTypeParameter>,
    private var innerClasses: SmartFMap<String, InnerClassInfo>
) {
    constructor(classesByQName: ClassIdToJavaClass) : this(classesByQName, SmartFMap.emptyMap(), SmartFMap.emptyMap())
    ...
    internal fun addInnerClass(innerInternalName: String, outerInternalName: String, simpleName: String) {
        innerClasses = innerClasses.plus(innerInternalName, InnerClassInfo(outerInternalName, simpleName))
    }
    ...
    internal fun addTypeParameters(newTypeParameters: Collection<JavaTypeParameter>) {
        ...
        typeParameters = newTypeParameters.fold(typeParameters) { acc, typeParameter ->
            acc.plus(typeParameter.name.identifier, typeParameter)
        }
    }

    private fun resolveClass(classId: ClassId) = Result(classesByQName(classId), classId.asSingleFqName().asString())
    internal fun resolveTypeParameter(name: String) = Result(typeParameters[name], name)
    ...
    internal fun mapInternalNameToClassId(internalName: String): ClassId {
        if ('$' in internalName) {
            val innerClassInfo = innerClasses[internalName]
